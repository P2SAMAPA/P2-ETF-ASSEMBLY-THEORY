...
from assembly_theory import assembly_theory_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        ...
        s = assembly_theory_score(
            ret_window[ticker].values,
            macro_window,
            primitive_size=config.PRIMITIVE_SIZE,
            max_steps=config.MAX_ASSEMBLY_STEPS
        )
    ...
    top_etfs = [{"ticker": t, "assembly_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "primitive_size": config.PRIMITIVE_SIZE,
        "max_assembly_steps": config.MAX_ASSEMBLY_STEPS,
    ...
    out_file = f"output/assembly_{timestamp}.json"
