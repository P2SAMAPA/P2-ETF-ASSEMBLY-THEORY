...
st.set_page_config(page_title="Assembly Theory – Market Complexity", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🧩 Assembly Theory – Market Complexity</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Assembly index of price patterns | Structured predictability | Elementary move construction</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Assembly Theory")
...
st.sidebar.markdown(f"**Primitive size:** {config.PRIMITIVE_SIZE} | **Max steps:** {config.MAX_ASSEMBLY_STEPS}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'assembly_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Assembly index: {etf['assembly_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Assembly Index"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Assembly Index", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🧩 Top ETFs by Assembly Index (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Assembly Theory** (recently in Nature) measures the complexity of objects.
        - The **assembly index** counts how many steps are needed to construct a price trajectory from elementary moves.
        - Higher assembly index → more structured, potentially predictable patterns.
        - Lower assembly index → simple, noisy, or random patterns.
        - Macro variables modulate the assembly complexity.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the assembly indices.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Assembly Theory | Assembly index for market pattern complexity")
